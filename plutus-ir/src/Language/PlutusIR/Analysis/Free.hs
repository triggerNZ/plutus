...
fTerm :: forall name tyname uni a.
        (Ord tyname, PLC.HasUnique tyname PLC.TypeUnique, PLC.HasUnique name PLC.TermUnique)
...
fTerm = \case
 ...
 where
  ...
  fLet bs tIn =
    let
      allIds = S.fromList $ bs^..traverse.bindingIds
  ...
  nonRecVarAcc (accLinearScope,accFreeSet) b =
    let newScope = S.fromList (b^..bindingIds) <> accLinearScope
...
fBinding :: forall name tyname uni a.
           (Ord tyname, PLC.HasUnique name PLC.TermUnique, PLC.HasUnique tyname PLC.TypeUnique)
...
-- See Note: [PIR Free variables]
fType :: (Ord tyname, PLC.HasUnique tyname PLC.TypeUnique)
