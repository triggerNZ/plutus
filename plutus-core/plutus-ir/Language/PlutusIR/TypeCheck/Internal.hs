...
{- Note [PIR vs paper FIR Difference-1]
...
{- Note [PIR vs paper FIR Difference-2]
Link to the paper: <https://hydra.iohk.io/job/Cardano/plutus/linux.papers.unraveling-recursion/latest/download-by-type/doc-pdf/unraveling-recursion>
...
In such a case we do not run this rule at all (Gamma !- inTermTopLevel :: *) and instead
allow the Type to escape its scope. The reasoning behind this is that such "escaping"-PIR programs would
behave correctly when they are translated down to PLC. For more info see : </plutus-core/docs/Typechecking PIR.md>
...
checkTypeM
    ...
    => ann -> Term TyName Name uni ann -> Normalized (Type TyName uni ()) -> TypeCheckM uni IsTop e ()
...
inferTypeM
    ...
    => Term TyName Name uni ann -> TypeCheckM uni IsTop e (Normalized (Type TyName uni ()))
-- c : vTy
-- -------------------------
-- [infer| G !- con c : vTy]
...
-}
inferTypeM (Let ann r@NonRec bs inTerm) =
    ...
    >>= through (checkStarInferred ann)
 where
   checkBindingThenScope :: Binding TyName Name uni ann -> TypeCheckM uni IsTop e res -> TypeCheckM uni IsTop e res
...
-}
inferTypeM (Let ann r@Rec bs inTerm) =
...
checkTypeFromBinding :: forall e uni a. (GShow uni, GEq uni, DefaultUni <: uni, AsTypeError e (Term TyName Name uni ()) uni a, AsTypeErrorExt e uni a)
                 => Recursivity -> Binding TyName Name uni a  -> TypeCheckM uni IsTop e ()
checkTypeFromBinding recurs = \case
    TypeBind{} -> pure () -- no types to check
    TermBind _ _ (VarDecl ann _ ty) rhs ->
        noEscapeType (checkTypeM ann rhs . fmap void =<< normalizeTypeM ty)
    DatatypeBind _ dt@(Datatype ann _ tyargs _ constrs) ->
      ...
      where
       ...
       checkConRes :: Type TyName uni a -> TypeCheckM uni o e ()
       checkConRes ty =
       ...
       checkNonRecScope :: Type TyName uni a -> TypeCheckM uni o e ()
...
-- Skip this check at the top-level, to allow top-level types to escape; see Note [PIR vs paper FIR Difference-2].
checkStarInferred :: AsTypeError e term uni ann
                  => ann -> Normalized (Type TyName uni b) -> TypeCheckM uni IsTop e ()
checkStarInferred ann t = do
    isTop <- asks view tceOther
    case isTop of
        NoTop  -> checkKindM ann (ann <$ unNormalized t) $ Type ()
...
data IsTop = YesTop | NoTop

-- | Changes the flag in nested-lets so to disallow returning a type outside of the type's scope
noEscapeType :: TypeCheckM uni IsTop e a -> TypeCheckM uni IsTop e a
noEscapeType = local $ tceOther .~ NoTop

-- | Run a 'TypeCheckM' computation by supplying a 'TypeCheckConfig' to it.
-- Differs from its PLC version in that is passes an extra env flag 'YesTop'.
runTypeCheckM :: (MonadError e m, MonadQuote m)
              => TypeCheckConfig uni -> TypeCheckM uni IsTop e a -> m a
runTypeCheckM config a =
    liftEither =<< liftQuote (runExceptT $ runReaderT a env) where
        env = TypeCheckEnv config mempty mempty YesTop
...
withVarsOfBinding :: forall uni o e a res.
                    Recursivity -> Binding TyName Name uni a
                  -> TypeCheckM uni o e res -> TypeCheckM uni o e res
...
withVarsOfBindings :: Foldable t => Recursivity -> t (Binding TyName Name uni a)
                   -> TypeCheckM uni o e res -> TypeCheckM uni o e res
withVarsOfBindings = flip . foldr . withVarsOfBinding

-- | Scope a typechecking computation with the given binding's newly-introducing type (if there is one)
withTyVarsOfBinding :: Binding TyName name uni ann -> TypeCheckM uni o e res -> TypeCheckM uni o e res
...
withTyVarsOfBindings :: Foldable f => f (Binding TyName name uni ann) -> TypeCheckM uni o e res -> TypeCheckM uni o e res
withTyVarsOfBindings = flip $ foldr withTyVarsOfBinding

-- | Helper to add type variables into a computation's environment.
withTyVarDecls :: [TyVarDecl TyName ann] -> TypeCheckM uni o e a -> TypeCheckM uni o e a
