...
type MSTrace m a = ContractTrace (SM.SMSchema MS.State MS.Input) (SM.SMContractError MS.State MS.Input) m a
...
lock' :: (MonadEmulator m) => Value -> MSTrace m a ()
...
proposeSignPay :: (MonadEmulator m) => Integer -> MSTrace m a ()
