...
let
  ...
  packages = self: (rec {
    ...
    marlowe-symbolic-static = 
	let
                pkgsMusl = pkgs.pkgsMusl;
                extension = sel: sup: { mkDerivation = drv: sup.mkDerivation (drv // { enableSharedExecutables = false;
	                                                                               enableSharedLibraries = false;
                								       enableStaticLibraries = true;
                								       isLibrary = true;
                								       isExecutable = false;
	                                                                               configureFlags = (if builtins.hasAttr "configureFlags" drv then drv.configureFlags else []) ++ 
												[ 
